...
/** 
 * A write attribute handler for the post-auth-redirect attribute.
 *
 * @author Carl Harris
 */
class PostAuthRedirectHandler extends AbstractProfileAttributeHandler<Void> {

  static final PostAuthRedirectHandler INSTANCE =
      new PostAuthRedirectHandler();
  
  private PostAuthRedirectHandler() {
    super(ProfileDefinition.POST_AUTH_REDIRECT);
  }
  ...
  protected Void applyUpdateToConfiguration(String attributeName,
    ...
    config.setPostAuthRedirect(value.resolve().asBoolean());
    return null;
  }
